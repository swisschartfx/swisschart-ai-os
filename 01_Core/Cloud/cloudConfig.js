...
function loadCloudConfig(environment = process.env) {
    ...
    if (environment.SWISSCHART_MCP_BEARER_TOKEN.length < 32) {
        const error = new Error(
            "SWISSCHART_MCP_BEARER_TOKEN must contain at least 32 characters"
        );
    ...
    try {
        ...
        const error = new Error(
            "SWISSCHART_PUBLIC_BASE_URL must be a valid HTTPS URL"
        );
    ...
    if (
        ...
        const error = new Error(
            "SWISSCHART_PUBLIC_BASE_URL must be an HTTPS origin"
        );
        error.code = "CLOUD_PUBLIC_URL_INVALID";
        throw error;
    }
    ...
    const oauthStateFile = environment.SWISSCHART_OAUTH_STATE_FILE;

    const scheduleDatabaseFile =
        typeof environment.SWISSCHART_SCHEDULE_DATABASE_FILE === "string" &&
        environment.SWISSCHART_SCHEDULE_DATABASE_FILE.trim()
            ? environment.SWISSCHART_SCHEDULE_DATABASE_FILE.trim()
            : path.join(
                path.dirname(oauthStateFile),
                "schedules.sqlite"
            );
