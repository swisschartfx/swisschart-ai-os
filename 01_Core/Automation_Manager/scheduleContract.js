...
function normalizeSchedule(input, options = {}) {
    ...
    if (!Number.isInteger(priority)) {
        throw coded(
            ...
            "Schedule priority must be an integer"
        );
    }
...
function normalizeWeekdays(value) {
    if (!Array.isArray(value) || value.length === 0) {
        throw coded(
            "SCHEDULE_WEEKDAYS_REQUIRED",
            "At least one ISO weekday is required"
        );
    }
    ...
    if (
        ...
        throw coded(
            ...
            "ISO weekdays must be integers from 1 to 7"
        );
    }
...
function normalizeTrigger(value) {
    if (!value || !TRIGGER_TYPES.includes(value.type)) {
        throw coded(
            ...
            "Unsupported schedule trigger type"
        );
    }
    ...
    if (!["reject", "earlier", "later"].includes(disambiguation)) {
        throw coded(
            "SCHEDULE_DISAMBIGUATION_INVALID",
            "Invalid DST disambiguation policy"
        );
    }
    ...
    if (
        ...
        Math.abs(offsetMinutes) > 1440
    ) {
        throw coded(
            "SCHEDULE_OFFSET_INVALID",
            "Offset minutes must be an integer within one day"
        );
    }
    ...
    const boundary = requiredString(
        value.boundary,
        "SCHEDULE_BOUNDARY_REQUIRED"
    );

    if (!["open", "close"].includes(boundary)) {
        throw coded(
            "SCHEDULE_BOUNDARY_INVALID",
            "Session boundary must be open or close"
        );
    }
    ...
    return {
        ...
        session,
        boundary,
        authoritativeLocalTime:
            normalizeTime(value.authoritativeLocalTime),
        timezone,
        offsetMinutes,
        disambiguation
    };
...
function normalizePublication(value) {
    if (
        !value ||
        value.destination !== "telegram.primary"
    ) {
        throw coded(
            "SCHEDULE_DESTINATION_INVALID",
            "Only telegram.primary is supported"
        );
    }
    ...
    if (
        ...
        throw coded(
            ...
            "Publication template is required"
        );
    }

    const content = requiredString(
        value.template.content,
        "SCHEDULE_TEMPLATE_CONTENT_REQUIRED"
    );

    const templateRevision =
        Number(value.template.revision);
    ...
    if (
        ...
        throw coded(
            "SCHEDULE_TEMPLATE_REVISION_INVALID",
            "Template revision must be positive"
        );
    }

    const displayTimezone =
        value.displayTimezone || DISPLAY_TIMEZONE;

    if (displayTimezone !== DISPLAY_TIMEZONE) {
        throw coded(
            "SCHEDULE_DISPLAY_TIMEZONE_INVALID",
            "Channel display timezone must be America/New_York"
        );
    }
    ...
    return {
        ...
        displayTimezone,
        rendererVersion: requiredString(
...
function normalizeExecutionPolicy(value = {}) {
    const misfireMode =
        value.misfireMode || "skip_and_record";

    if (misfireMode !== "skip_and_record") {
        throw coded(
            "SCHEDULE_MISFIRE_MODE_INVALID",
            "Only skip_and_record is supported"
        );
    }
    ...
    if (
        ...
        throw coded(
            "SCHEDULE_MISFIRE_GRACE_INVALID",
            "Misfire grace must be null or a non-negative integer"
        );
    }

    const holidayPolicy =
        value.holidayPolicy || "none";

    if (holidayPolicy !== "none") {
        throw coded(
            "SCHEDULE_HOLIDAY_POLICY_INVALID",
            "Holiday intelligence is not implemented"
        );
    }
...
function occurrenceIdentity(
    ...
    const identity =
        `schedule:${schedule.scheduleId}:revision:${schedule.revision}:` +
        `local-date:${localDate}:trigger:${resolvedInstant}`;

    return {
        identity,
        occurrenceKey: sha256(identity)
    };
}
...
function normalizeTime(value) {
    if (
        ...
        throw coded(
            ...
            "Local wall time must use HH:MM"
        );
    }
...
function validateTimezone(timezone) {
    try {
        ...
        throw coded(
            ...
            "Timezone must be a valid IANA identity"
        );
    }
}
