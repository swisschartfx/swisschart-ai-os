...
## Rule 2 — Capability and Agent Responsibility

Capabilities own reusable business or technical responsibilities. A Capability does not require an Agent.

Agents are optional specialized implementation components used only when an independent actor, workflow lifecycle, specialized worker or orchestration behavior is required.
...
Agents must not become isolated mini-systems, competing business authorities or independent shared-intelligence systems.
...
## Rule 6 — Architectural Definition and Capability Completion

Swisschart AI OS uses Top-Down Architectural Definition and Bottom-Up Capability Completion.

- Define stable interface, authority, orchestration, capability, execution-control, data-boundary and provider contracts in advance.
- Implement real capabilities end-to-end using only shared infrastructure they actually require.
- Treat architecture as a dependency graph, not a mandatory processing pipeline.
- Do not create physical folders, packages, services or monoliths merely to mirror logical architecture.
- Do not make future Agents prerequisites for an existing capability's production completion.
- Do not pre-build hypothetical universal infrastructure.

## Rule 7 — Parallel Concerns and Explicit Ownership

Shared Core, Domain Capabilities and Execution & Control Infrastructure are parallel logical concerns, not a vertical stack. They may depend on one another only through explicit contracts.

Canonical ownership must be explicit for Data, Metrics, Identity, Policy, Operational State and Shared Intelligence. No Agent or domain may silently become a competing authority.

Technical duplication such as caches, local projections, provider-specific logic and specialized analytics is allowed when ownership remains explicit. Competing business authority, canonical truth, canonical metrics, shared intelligence or execution-control authority is prohibited.

## Rule 8 — Operational State and Memory

Operational State includes approvals, tasks, events, scheduler occurrences, automation state, execution records, idempotency and durable workflow/business state. It must remain deterministic and durable and must never be treated as AI memory.

Context / Memory may hold conversational context, prior Founder decisions and reasoning/workflow-context references. It must not replace authoritative Operational State.

## Rule 9 — Analytics and Entity References

Domain-specific analytics are allowed. Canonical metric definitions, shared business intelligence, cross-domain analytics and authoritative interpretations require explicit ownership and must not be independently and inconsistently reimplemented across Agents.

Future domain entities should be able to expose normalized identity/entity references, but no universal schema is authorized until real domain requirements justify it.
